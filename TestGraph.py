...
#     def tangles_test(self, generated_graph, generated_ground_truth, agreement_parameter, k, initial_partition_method="K-Means", cost_function="Kernighan-Lin Cost Function"):
#         try:
#             data = DataSetGraph(agreement_param=agreement_parameter, k=k)
#             data.G = generated_graph
#             start_time = time.time()
...
#             data.generate_multiple_cuts(data.G, initial_partition_method=initial_partition_method)
#             end_time_kernighan = time.time()
#             data.cost_function_Graph(cost_function=cost_function)
#             root = create_searchtree(data)
...
if __name__ == "__main__":
    ...
    for num_of_nodes in num_of_nodes_list:
        ...
        for _ in range(num_iterations):
            ...
            # Test 3
            algorithms = ['Kernighan-Lin Cost Function', 'Modularity cost', 'Edge cut cost']
            running_times = []
            nmi_scores = []

            for algorithm in algorithms:
                if algorithm == 'Kernighan-Lin Cost Function':
                    tangles_time, tangles_kmeans_time, tangles_nmi = test.tangles_test(generated_graph, generated_ground_truth,
                                                                     agreement_parameter[num_of_nodes_list.index(num_of_nodes)], k, initial_partition_method="K-Means", cost_function="Kernighan-Lin Cost Function")
                    nmi_scores.append(tangles_nmi)
                    running_times.append(tangles_time)
                elif algorithm == 'Modularity cost':
                    tangles_time, tangles_kmeans_half_time, tangles_nmi = test.tangles_test(generated_graph, generated_ground_truth,
                                                                        agreement_parameter[num_of_nodes_list.index(num_of_nodes)], k, initial_partition_method="K-Means", cost_function="Modularity cost")
                    nmi_scores.append(tangles_nmi)
                    running_times.append(tangles_time)
                elif algorithm == 'Edge cut cost':
                    tangles_time, tangles_kmeans_both_time, tangles_nmi = test.tangles_test(generated_graph, generated_ground_truth,
                                                                     agreement_parameter[num_of_nodes_list.index(num_of_nodes)], k, initial_partition_method="K-Means", cost_function="Edge cut cost")
                    nmi_scores.append(tangles_nmi)
                    running_times.append(tangles_time)
    ...
    plt.xlabel('Number of Nodes')
    plt.ylabel('Average NMI Score')
    plt.title('Average NMI Scores of Different Cost Functions ({} iterations)'.format(num_iterations))
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.xlabel('Number of Nodes')
    plt.ylabel('Average Running Time (s)')
    plt.title('Average Running Times of Different Cost FUnctions ({} iterations)'.format(num_iterations))
    plt.legend()
    plt.grid(True)
    plt.show()
